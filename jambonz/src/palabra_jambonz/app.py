"""FastAPI application: ``/stt`` (ws), ``/tts`` (POST), ``/healthz`` (GET)."""
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title='palabra-jambonz-bridge', version=__version__, docs_url=None, redoc_url=None)
    ...
    async def stt(ws: WebSocket) -> None:
        await handle_stt(ws, settings, factory)

    @app.post('/tts')
    async def tts(request: Request) -> Response:
        return await handle_tts(request, settings, factory)
